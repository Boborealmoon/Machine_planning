    ...
    return rows(
        con.execute(
            ...
            SELECT b.block_id, b.machine_id, b.group_id, b.execution_status, b.anchor_datetime,
                   b.active, o.source_ps_id, o.job_no
...
def block_ready_for_auto_unschedule(con, block_id: int) -> bool:
    block = one(
        con.execute(
            """
            SELECT b.block_id, b.group_id, b.execution_status, b.active
            FROM planner_run_block b
            WHERE b.block_id = %s
            """,
            (int(block_id),),
    ...
    if not members:
        return False
    return all(compact_text(row.get("execution_status")).upper() == "DONE" for row in members)
...
def unschedule_done_block(con, block_id: int, *, reason: str = "AUTO_DONE") -> dict:
    ...
    if group_id > 0:
        ...
        if not all(compact_text(row.get("execution_status")).upper() == "DONE" for row in member_rows):
        ...
        if compact_text(block.get("execution_status")).upper() != "DONE":
