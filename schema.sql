...
CREATE TABLE IF NOT EXISTS bom_variation (
  ...
  created_at TEXT DEFAULT CURRENT_TIMESTAMP,
  updated_at TEXT DEFAULT CURRENT_TIMESTAMP
  -- Removed UNIQUE(part_id, bom_code) as bom_code is now globally unique PK
);

CREATE TABLE IF NOT EXISTS operation_seq (
  op_seq_id INTEGER PRIMARY KEY AUTOINCREMENT,
  bom_code TEXT NOT NULL REFERENCES bom_variation(bom_code) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS process_sheet (
  ...
  selected_bom_code TEXT REFERENCES bom_variation(bom_code),
...
CREATE TABLE IF NOT EXISTS bom_material (
  ...
  bom_code TEXT REFERENCES bom_variation(bom_code),
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_machines_machine_code_unique ON machines(machine_code);
CREATE UNIQUE INDEX IF NOT EXISTS idx_capacity_profile_profile_name_unique ON capacity_profile(profile_name);
CREATE INDEX IF NOT EXISTS idx_bom_variation_part_id ON bom_variation(part_id);